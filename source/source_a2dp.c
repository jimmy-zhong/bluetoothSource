...
void a2dp_init(void)
{
    ...
    if (theSource->ps_config->a2dp_codecs.aptxhd)
    {
        seps[num_endpoints].in_use = FALSE;
        seps[num_endpoints++].sep_config = &theSource->a2dp_data.aptxhd_caps; 
    }
...
void a2dp_set_aptxhd_config(bool use_defaults)
{
    if (use_defaults)
    {
        theSource->a2dp_data.aptxhd_codec_config = NULL; /* no change in configuration */
        theSource->a2dp_data.aptxhd_caps = aptxhd_sep;   /* default End Point data */
    }
    else
    {
        if (theSource->a2dp_data.aptxhd_codec_config)
        {        
            uint16 i;
            /* copy the original codec settings to the memory location */
            for (i = 0; i < sizeof(a2dp_aptxhd_caps_source); i++)
            { 
                theSource->a2dp_data.aptxhd_codec_config[i] = a2dp_aptxhd_caps_source[i];
            }

            /* change as defined in PS Key data */
            theSource->a2dp_data.aptxhd_codec_config[A2DP_APTX_SAMPLING_RATE_INDEX] = (theSource->ps_config->aptx_config.sampling_freqs & 0xf0) | A2DP_APTX_CHANNEL_MODE_STEREO;                        
            /* update with the new caps */
            theSource->a2dp_data.aptxhd_caps.caps = theSource->a2dp_data.aptxhd_codec_config;           
        }
    }
}
...
bool a2dp_configure_aptxhd(A2DP_CODEC_CONFIGURE_IND_T *message)
{
    ...
    /* Check Sample Rates */
    if ((theSource->a2dp_data.aptxhd_caps.caps[local_index] & 0x10) &&
        (message->codec_service_caps[remote_index] & 0x10))
    {
        /* 48kHz */
        message->codec_service_caps[remote_index] &= 0x1f;
    }
    else if ((theSource->a2dp_data.aptxhd_caps.caps[local_index] & 0x20) &&
             (message->codec_service_caps[remote_index] & 0x20))
    {
        /* 44.1kHz */
        message->codec_service_caps[remote_index] &= 0x2f;
    }
    else if ((theSource->a2dp_data.aptxhd_caps.caps[local_index] & 0x40) &&
             (message->codec_service_caps[remote_index] & 0x40))
    {
        /* 32kHz */
        message->codec_service_caps[remote_index] &= 0x4f;
    }
    else if ((theSource->a2dp_data.aptxhd_caps.caps[local_index] & 0x80) &&
             (message->codec_service_caps[remote_index] & 0x80))
    {
        /* 16kHz */
        message->codec_service_caps[remote_index] &= 0x8f;
    ...
    /* Select Channel Mode */
    if ((theSource->a2dp_data.aptxhd_caps.caps[local_index] & 0x01) && 
        (message->codec_service_caps[remote_index] & 0x01))
    {
        /* choose joint stereo */
        message->codec_service_caps[remote_index] &= 0xf1;
    }
    else if ((theSource->a2dp_data.aptxhd_caps.caps[local_index] & 0x02) && 
             (message->codec_service_caps[remote_index] & 0x02))
    {
        /* choose stereo */
        message->codec_service_caps[remote_index] &= 0xf2;
    }
    else if ((theSource->a2dp_data.aptxhd_caps.caps[local_index] & 0x04) && 
             (message->codec_service_caps[remote_index] & 0x04))
    {
        /* choose dual mode */
        message->codec_service_caps[remote_index] &= 0xf4;
    }
    else if ((theSource->a2dp_data.aptxhd_caps.caps[local_index] & 0x08) && 
             (message->codec_service_caps[remote_index] & 0x08))
    {
        /* choose dual mode */
        message->codec_service_caps[remote_index] &= 0xf8;
...
{    
        ...
        (old_state == A2DP_STATE_CONNECTING_REMOTE))
    {
        ...
        ConnectionReadRemoteVersionBdaddr(&theSource->connectionTask, &tpaddr); 
    ...
    else
    {
        ...
        else
        {
            ...
            MessageSend(&inst->a2dpTask, A2DP_INTERNAL_SIGNALLING_DISCONNECT_REQ, 0);
             A2DP_DEBUG(("A2DP disconnecting a2dp_enter_state_connected_signalling\n"));
        }        
    }        
...
static void a2dp_enter_state_connecting_media_local(a2dpInstance *inst, A2DP_STATE_T old_state)
{
    ...
    else
    {
        ...
        if (theSource->ps_config->a2dp_codecs.aptxhd)
        {
            ...
            a2dp_seid_preference[current_endpoint++] = theSource->ps_config->a2dp_codecs.aptxhd_preference;
        } 
        if (theSource->ps_config->a2dp_codecs.aptxLowLatency)
        {
    ...
    {
        MessageSend(&inst->a2dpTask, A2DP_INTERNAL_SIGNALLING_DISCONNECT_REQ, 0);
        A2DP_DEBUG(("A2DP disconnecting a2dp_enter_state_connecting_media_local"));
    }
}
...
static void a2dp_enter_state_connected_media(a2dpInstance *inst, A2DP_STATE_T old_state)
{
    ...
    {                
        ...
        else
        {
            ...
            MessageSend(&inst->a2dpTask, A2DP_INTERNAL_SIGNALLING_DISCONNECT_REQ, 0);
            A2DP_DEBUG(("A2DP disconnecting a2dp_enter_state_connected_media"));
        }
    }
}
...
static void a2dp_enter_state_connected_media_suspending_local(a2dpInstance *inst, A2DP_STATE_T old_state)
{
    ...
    {
        MessageSend(&inst->a2dpTask, A2DP_INTERNAL_SIGNALLING_DISCONNECT_REQ, 0);
        A2DP_DEBUG(("A2DP disconnecting a2dp_enter_state_connected_media_suspending_local"));
    }
    else
    {
...
static void a2dp_enter_state_connected_media_starting_local(a2dpInstance *inst, A2DP_STATE_T old_state)
{
    ...
    {
        MessageSend(&inst->a2dpTask, A2DP_INTERNAL_SIGNALLING_DISCONNECT_REQ, 0);
        A2DP_DEBUG(("A2DP disconnecting a2dp_enter_state_connected_media_starting_local"));
    }
}
