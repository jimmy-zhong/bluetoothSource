...
{    
    ...
    aptxhd_config = ps_read_user(PS_KEY_USER_APTXHD_CONFIG, &theSource->ps_config->aptxhd_config, sizeof(PS_APTXHD_CONFIG_T));
    ...
    if (sbc_config || faststream_config || aptx_config || aptxhd_config || aptxLowLatency_config)
    {
        ...
        if (aptxhd_config)
        {
            theSource->a2dp_data.aptxhd_codec_config = (uint8 *)memory_get_block(MEMORY_GET_BLOCK_CODEC_APTXHD);
