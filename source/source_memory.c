...
bool memory_create_block(MEMORY_CREATE_BLOCK_T block)
{
    ...
    switch (block)
    {
        ...
        {
            theSource->a2dp_data.codec_config = memory_create(a2dp_get_sbc_caps_size() + a2dp_get_faststream_caps_size() + a2dp_get_aptx_caps_size()  + a2dp_get_aptxhd_caps_size() + a2dp_get_aptxLowLatency_caps_size());
      ...
      case MEMORY_GET_BLOCK_CODEC_APTX_LOW_LATENCY:
        {
            if (theSource->a2dp_data.codec_config)
            {
                return theSource->a2dp_data.codec_config + a2dp_get_sbc_caps_size() + a2dp_get_faststream_caps_size()  + a2dp_get_aptx_caps_size() + a2dp_get_aptxhd_caps_size();
            }
        }
        break;
